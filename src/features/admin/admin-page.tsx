...
export function AdminPage({
  ...
  if (error || !data) {
    return (
      <section className="section-pad content-page">
        <p className="eyebrow">Адміністрування</p>
        <h1>Панель недоступна</h1>
        <div className="empty-state">
          <p>{error || "Не вдалося отримати дані."}</p>
  ...
  const renameSubject = async (subject: Subject): Promise<void> => {
    ...
    await onExecute(
      "subject-update",
      () => api.updateSubject(subject.id, name.trim()),
      "Назву предмета оновлено."
    );
  };
  ...
  const submitQuiz = async (event: SubmitEvent<HTMLFormElement>): Promise<void> => {
    ...
    const operation =
      draftId !== null ? () => api.updateQuiz(draftId, payload) : () => api.createQuiz(payload);
    const result = await onExecute(
      "quiz-save",
      operation,
      draftId !== null ? "Тест оновлено." : "Тест створено."
    );
    if (result)
      setQuizDraft({
        id: null,
        name: "",
        subjectId: String(data.subjects[0]?.id || ""),
        levelId: String(data.levels[0]?.id || ""),
        timeToPassMinutes: 10
      });
  };
  const editQuiz = (quiz: AdminQuiz): void =>
    setQuizDraft({
      id: quiz.id,
      name: quiz.name,
  ...
  const submitQuestion = async (event: SubmitEvent<HTMLFormElement>): Promise<void> => {
    ...
    const result = await onExecute(
      "question-save",
      operation,
      draftId !== null ? "Запитання оновлено." : "Запитання додано."
    );
    if (result) {
  ...
  const editQuestion = (question: AdminQuestion): void =>
    setQuestionDraft({
      id: question.id,
      text: question.text,
  ...
  const changeUserStatus = async (user: AdminUser): Promise<void> => {
    ...
    await onExecute(
      "user-status",
      () => api.updateUserStatus(user.id, nextStatus),
  ...
  return (
    <>
      <section className="page-hero section-pad admin-hero">
        <p className="eyebrow">P9 · React administration</p>
        <h1>
          Керуйте платформою
          <br />
          <em>без legacy UI.</em>
        ...
        <p>
          Предмети, тести, запитання, користувачі та результати тепер працюють через захищений Spring Boot
          REST API.
        </p>
      </section>
      <section className="section-pad admin-dashboard">
        <div className="admin-stats">
          ...
          <div>
            ...
            <strong>{data.usersPage?.totalCount ?? data.users.length}</strong>
          </div>
          <div>
            <span>Результати</span>
            <strong>{data.resultsPage?.totalCount ?? data.results.length}</strong>
          </div>
        </div>

        <AdminSection eyebrow="Каталог" title="Предмети">
          <form className="admin-inline-form" onSubmit={createSubject}>
            <label>
              <span className="sr-only">Назва нового предмета</span>
              <input
                required
                maxLength={25}
                value={subjectName}
          ...
          <div className="admin-list">
            {data.subjects.map(subject => (
              <div className="admin-list__row" key={subject.id}>
                <div>
                  <span className="admin-id">#{subject.id}</span>
        ...
        <AdminSection eyebrow="Контент" title="Тести">
          <form className="admin-grid-form" onSubmit={submitQuiz}>
            <label>
              ...
              <input
                required
                maxLength={50}
                value={quizDraft.name}
                onChange={event => setQuizDraft({ ...quizDraft, name: event.target.value })}
              />
            </label>
            <label>
              <span>Предмет</span>
              <select
                ...
                onChange={event => setQuizDraft({ ...quizDraft, subjectId: event.target.value })}
              >
                {data.subjects.map(subject => (
            ...
            <label>
              ...
              <select
                required
                value={quizDraft.levelId}
                onChange={event => setQuizDraft({ ...quizDraft, levelId: event.target.value })}
              >
            ...
            <label>
              ...
              <input
                ...
                max="1440"
                type="number"
                value={quizDraft.timeToPassMinutes}
                onChange={event => setQuizDraft({ ...quizDraft, timeToPassMinutes: event.target.value })}
              />
            </label>
            <div className="button-row">
              <button className="button button--dark" type="submit" disabled={working}>
                {quizDraft.id ? "Зберегти" : "Створити тест"}
              </button>
              {quizDraft.id && (
                <button
          ...
          <div className="admin-table">
            ...
            {data.quizzes.map(quiz => (
              <div className="admin-table__row" key={quiz.id}>
                <div>
                  <strong>{quiz.name}</strong>
                  <small>{quiz.subject}</small>
                </div>
                <span>
                  {difficultyLabel(quiz.complexity)} · {quiz.timeToPassMinutes} хв · {quiz.totalQuestions}{" "}
                  зап.
                </span>
                <div className="button-row">
                  <button
        ...
        >
          {!selectedQuizId ? (
            <div className="empty-state">
              <p>Спочатку створіть тест.</p>
            </div>
          ) : (
            <>
              <form className="admin-question-form" onSubmit={submitQuestion}>
                <label>
                  <span>Текст запитання</span>
                  <textarea
                    required
                    maxLength={250}
                ...
                <div className="admin-answer-grid">
                  {questionDraft.answers.map((answer, index) => (
                    <label key={answer.key}>
                      <span>Варіант {index + 1}</span>
                      <input
                        required
                        maxLength={50}
                        value={answer.text}
                        onChange={event => changeAnswer(answer.key, "text", event.target.value)}
                      />
                      <span className="admin-check">
                        <input
                          type="checkbox"
                          checked={answer.correct}
                          onChange={event => changeAnswer(answer.key, "correct", event.target.checked)}
                        />{" "}
                        Правильна відповідь
                      </span>
                    </label>
                ...
                <div className="button-row">
                  <button className="button button--dark" type="submit" disabled={working}>
                    {questionDraft.id ? "Зберегти запитання" : "Додати запитання"}
                  </button>
                  {questionDraft.id && (
                    <button
                      className="button button--ghost"
                      type="button"
                      onClick={() => setQuestionDraft({ id: null, text: "", answers: blankAnswers() })}
                    >
                      Скасувати
                    </button>
                  )}
              ...
              {questionError && <div className="alert alert--error">{questionError}</div>}
              {questionLoading ? (
                    ...
                    <article key={question.id}>
                      ...
                      <ol>
                        {question.answers.map(answer => (
                          <li className={answer.correct ? "is-correct" : ""} key={answer.id}>
                            {answer.text}
                            {answer.correct && " ✓"}
                          </li>
                        ))}
        ...
        <AdminSection eyebrow="Доступ" title="Користувачі">
          <div className="admin-table">
            <div className="admin-table__head">
              <span>Користувач</span>
              <span>Роль і статус</span>
              <span>Дія</span>
            </div>
            {data.users.map(user => (
              <div className="admin-table__row" key={user.id}>
                ...
                <span>
                  {user.role} ·{" "}
                  <b className={`status-dot status-dot--${user.status.toLowerCase()}`}>{user.status}</b>
                </span>
                <button
                  className="button button--ghost button--small"
                  type="button"
                ...
                >
                  {user.status.toLowerCase() === "active" ? "Заблокувати" : "Активувати"}
                </button>
              </div>
            ))}
          </div>
          ...
          action={
            <div className="admin-date-filter">
              <label>
                ...
                <input
                  type="datetime-local"
                  value={resultRange.from}
                  onChange={event => onResultRangeChange({ from: event.target.value })}
                />
              </label>
              <label>
                До
                <input
                  type="datetime-local"
                  value={resultRange.to}
                  onChange={event => onResultRangeChange({ to: event.target.value })}
                />
              </label>
            </div>
          }
        >
          <div className="admin-table">
            <div className="admin-table__head">
              <span>Користувач і тест</span>
              <span>Дата</span>
              <span>Результат</span>
            </div>
            {data.results.map(result => (
              <div className="admin-table__row" key={result.attemptId}>
                <div>
                  <strong>{result.username}</strong>
                  <small>
                    {result.quizName} · спроба #{result.attemptId}
                  </small>
                </div>
          ...
          <Pager
            meta={data.resultsPage}
            onChange={onResultsPageChange}
            busy={working || loading}
            label="Результати"
          />
          {!data.results.length && <p className="admin-empty">У вибраному діапазоні результатів немає.</p>}
        </AdminSection>
      </section>
    </>
  );
