...
api.ts          | 100.00 |    99.15 |  100.00
App.tsx         | 100.00 |    89.72 |  100.00
clock.ts        | 100.00 |   100.00 |  100.00
components.tsx  | 100.00 |   100.00 |  100.00
session.ts      | 100.00 |   100.00 |  100.00
utils.ts        | 100.00 |   100.00 |  100.00
all files       | 100.00 |    96.30 |  100.00
```
...
всередині або повертають, або кидають, тож V8 бачить продовження як недосяжний код. Решта
10% у `App.tsx` — здебільшого умови всередині JSX; їх варто закривати тестом, а не порогом.
...
```text
src/
  App.tsx              # стан застосунку, маршрути та API-сценарії
  components.tsx       # React-компоненти сторінок і повторно використовувані блоки
