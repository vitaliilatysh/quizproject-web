...
**Два різні запуски, і різниця між ними суттєва.**

`npm run coverage` — той самий збір, але з `--enable-source-maps`; хук віддає inline-мапу.
Без цієї пари числа виглядають правдоподібно й брешуть: покриття рахує рядки вже стертого
коду й приписує їх оригіналу. Було видно `request<T>` як «не покрито» — метод, через який
проходять усі тести до одного. Це запуск для **пошуку** прогалин: атрибуція правильна.

`npm run coverage:check` — без прапорця, і це не недогляд. Під прапорцем у знаменник
`line %` потрапляють рядки без відображення, насамперед коментарі в шапці файлу: кожен
модуль тут читається на кілька відсотків нижче за повного покриття свого коду, а `clock.ts`
показує 70% при 100% своїх функцій. Без прапорця знаменник — виконувані рядки, тобто рівно
те, що рахує лічильник `LINE` у JaCoCo, і рівно те, для чого поріг у 100% має сенс.
