...
Команда `npm run check` послідовно запускає перевірку типів, тести й production-збірку.
Окремо: `npm run typecheck` — лише `tsc --noEmit`.
...
- `dom.ts` — вікно happy-dom плюс `render`, `click` і `type`, кожен усередині `act()`,
...
`npm run coverage` — вбудований збір покриття Node, файли самих тестів виключено.

Обидва скрипти передають `--enable-source-maps`, а хук віддає inline-мапу. Без цієї пари
числа виглядають правдоподібно й брешуть: покриття рахує рядки вже стертого коду й
приписує їх оригіналу. Було видно `request<T>` як «не покрито» — метод, через який
проходять усі 85 тестів.

Одне застереження, яке варто знати, читаючи звіт: **`line %` занижений**. Рядки без
відображення — насамперед коментарі в шапці файлу — потрапляють у знаменник, тож рясно
прокоментований модуль показує низький відсоток навіть за повного покриття свого коду.
`branch %` і `funcs %` цієї вади не мають і як зведення надійніші.
