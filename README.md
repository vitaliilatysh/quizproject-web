...
`App.tsx` є композиційним шаром: визначає поточний екран і з'єднує його з feature hooks,
але не виконує API-запити та не зберігає стан окремих сценаріїв. Код згруповано за
відповідальністю:

```text
src/
  app/                 # навігація, повідомлення про помилки, toast і refresh сесії
  components/          # екрани й спільні presentation-компоненти за областями
  features/
    account/           # профіль та історія результатів
    admin/             # екран, контракти, дані й мутації адміністративної панелі
    attempts/          # життєвий цикл спроби та збережені відповіді
    auth/              # сесія, вхід, реєстрація та зміна пароля
    catalogue/         # пошук, фільтрація та пагінація тестів
    settings/          # адреса API та перевірка з'єднання
  App.tsx              # композиція маршрутів і feature-модулів
  components.tsx       # сумісний barrel для зовнішніх імпортів
```

Стан кожної можливості належить її hook. Спільними залишаються лише поточна сесія, API-клієнт
і індикатор активної дії; зміна користувача централізовано скидає всі дані попереднього
облікового запису.
...
- короткоживучий JWT та непрозорий refresh token зберігаються в `sessionStorage` і
  видаляються після закриття вкладки;
- поки відкрита вкладка з активною сесією, frontend тихо обмінює refresh token через
  ...
  завершується лише коли backend явно відхиляє refresh token; після успішного обміну frontend
  зберігає обидва ротовані токени;
- вихід викликає `POST /api/v1/auth/logout`, щоб backend відкликав серверну сесію, після чого
  локальні токени видаляються незалежно від результату мережевого запиту;
...
виконувати React-код, `tests/support/` додає рівно три речі:

- `jsx.mts` — module hook, який віддає кожен `.ts`/`.tsx` у esbuild на вході. Це той самий
  трансформ, що Vite застосовує до тих самих файлів під час збірки, тож тести виконують
  компоненти як вони написані, а не їхню копію. Він також переписує відносний імпорт
  `./api.js` на сусідній `api.ts` — Node резолвить специфікатор буквально, а джерела
  іменують файли так, як їх називатиме збірка;
- `browser-globals.ts` — вікно happy-dom, встановлене в глобали **до** імпорту react-dom.
  Порядок тут не косметичний: react-dom один раз, під час обчислення модуля, вимірює своє
  оточення — зокрема, чи взагалі надходять події `input`. Імпортований у голий Node, він
  записує «ні» й далі веде кожне текстове поле запасним шляхом, який до `onChange` не
  доходить. Саме це тут і було: введення в кероване поле міняло DOM і лишало стан
  компонента порожнім, тож проходили тільки тести над некерованими формами — рівно та
  множина тестів, яка існувала;
- `dom.ts` — `render`, `click`, `type`, `select`, `submit` і `settle`, кожен усередині
  `act()`, тож після виклику ефекти вже відпрацювали, а DOM усталився.

esbuild **стирає** типи, а не перевіряє їх — саме тому `npm run typecheck` існує окремою
командою й окремим кроком у CI. Без нього помилку типу не помітив би ніхто.

### Покриття

Поточний обов'язковий поріг: 100% рядків, 100% функцій і щонайменше 96% гілок для всього
`src/`. Фактично на `main` — 100% рядків (2006 із 2006), 100% функцій і 97.67% гілок. Звіт
охоплює як базові модулі, так і всі hooks у `src/app/` та `src/features/`.
...
(там правило `LINE` з `minimum = 1.0`), гілки — на досягнутому рівні, щоб число могло лише
зростати.

Одну гілку не закриє жоден тест: блок після `try/finally` у `QuizApi.send` — усі шляхи
всередині або повертають, або кидають, тож V8 бачить продовження як недосяжний код. Інші
непокриті гілки — захисні умови та browser fallback-и у feature hooks; їх варто закривати
тестом, а не зменшенням порогу.

**Рядки рахує не Node, і на це є причина.**

Тести виконують не той файл, що лежить на диску: module hook у `tests/support/jsx.mts`
проганяє кожен `.ts`/`.tsx` через esbuild. V8 звітує зсуви у згенерованому коді, а звіт
Node пише проти оригіналу. Inline-мапа закриває більшу частину розриву, але рядок оригіналу,
якому не відповідає жоден згенерований код, показати виконаним неможливо — і Node однаково
зараховує його в знаменник. Через це `line %` рухався від правок, що не чіпають жодної
інструкції. Заміряно на `2c47d3b`, змінювався лише коментар усередині одного ефекту:

| коментар усередині ефекту | `line %` від Node |
| --- | --- |
| немає | 100.00% |
| 1 рядок | 99.85% |
| 7 рядків, короткий текст | 99.96% |
| 7 рядків, довший текст | 99.78% |

Немонотонно — отже коментарі не рахувалися ніколи, це помилка зіставлення, а коментарі лише
зсувають, які рядки під неї потрапляють. Поріг у 100% на такому числі падає з причин, не
пов'язаних із тестами, а найпростіший спосіб його задовольнити — винести пояснення з функції,
яку воно пояснює.

Тому рядки гейтить `scripts/coverage-lines.mts`. Він читає сирий звіт V8
(`NODE_V8_COVERAGE=.coverage-v8`), перетранспільовує кожен файл тими самими опціями, що й
hook, і рахує навпаки: рядок оригіналу **виконуваний**, якщо мапа кладе на нього згенерований
код, і **покритий**, якщо той код виконувався. Рядки, яких мапа не знає, не рахуються взагалі —
їх не дістане жоден тест. Звіт V8 читається окремо для кожного тестового процесу й
об'єднується: V8 малює різні діапазони в різних процесах, і при злитті вузький нульовий
діапазон з одного процесу перекриває покривний з іншого.

Функції та гілки лишаються за Node — вони рахуються по діапазонах, а не по рядках, і цієї
вади не мають.

`npm run coverage` — збір із `--enable-source-maps` і без порогів. Атрибуція там правильна, і
це запуск для **пошуку** прогалин: він показує, у якому файлі й на якому рядку діра. Числа в
ньому свідомо занижені тим самим ефектом — рядки без відображення, насамперед шапка файлу,
йдуть у знаменник, — тож порогом воно бути не може.

І esbuild, і happy-dom — `devDependencies`; у бандл не потрапляє нічого.

Це покриває те, чого не дістає статичний рендер: **ефекти**. Критичні вади в оркестрації
frontend були саме в hooks — ефект, що витирав чернетки читача, коли не мав;
...
Ingress використовує один origin: `/` спрямовується до React, а `/api` — до Spring Boot.
Management endpoints backend залишаються на внутрішньому порту `9081` і не публікуються через
frontend Ingress. Kubernetes ConfigMap замінює `runtime-config.js` без повторної збірки frontend
і встановлює API URL у поточний public origin.
...
- тихе оновлення JWT (P20): workflow запускає backend із коротким `JWT_TTL`, тож сесія
  переживає завершення терміну дії початкового токена лише завдяки фоновому
  `POST /api/v1/auth/refresh` з ротацією непрозорого refresh token, а не повторному логіну.
...
scripts/               # prepare-hosting.mts — post-build крок для Sites/Cloudflare
                       # coverage-lines.mts — порогова перевірка покриття рядків
...
за тим, як ці відповіді читає фронтенд. Джерело кожного типу названо коментарем над ним,
щоб зміну на тому боці можна було простежити до типу, який має рушити слідом.
